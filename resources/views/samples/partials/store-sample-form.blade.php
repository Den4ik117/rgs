...
{{--    </form>--}}
    ...
    <form id="sampleForm" method="post" action="{{ route('samples.store') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-input-label for="name" :value="__('Сколько колонок с парами будет в первой таблице')" />
...
{{--        </div>--}}
        ...
        <div>
            <x-input-label for="is_public" class="flex items-center gap-2 cursor-pointer select-none">
                <input type="checkbox" name="is_public" id="is_public" @checked(old('is_public', true))>
