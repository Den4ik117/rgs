<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            {{ __("Update sample with random values X and Y.") }}
        </p>
    </header>
...
{{--    </form>--}}
    ...
    <form id="sampleForm" method="post" action="{{ route('samples.update', $sample->id) }}" class="mt-6 space-y-6">
        @csrf
        @method('PUT')

        <div>
            <x-input-label for="chunk" :value="__('Название выборки')" />
            <x-text-input id="chunk" name="name" type="text" class="mt-1 block w-full" :value="old('name', $sample->name)" required autofocus autocomplete="off" />
            <x-input-error class="mt-2" :messages="$errors->get('name')" />
        </div>

        <div>
            <div class="sample" data-data="{{ old('samples', $sample->values->map(fn ($value) => ['x' => $value->x, 'y' => $value->y])->toJson()) }}"></div>
        ...
        <div>
            ...
            <x-text-input id="name" name="chunk" type="number" min="1" max="20" step="1" class="mt-1 block w-full" :value="old('chunk', $sample->chunk)" required autocomplete="off" />
...
{{--            <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $sample->email->email)" required autocomplete="email" />--}}
