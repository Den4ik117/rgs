        ...
        @foreach($report->x->intervals as $interval)
            <tr>
                <td class="border px-6 py-4 text-center">
                    $ {{ ($loop->first ? '[' : '(') . $interval->interval[0] . '; ' . $interval->interval[1] . ']' }} $
                </td>
...
{{--         data-data="{{ $report->x->intervals->map(fn ($in) => ['x' => $in->interval[1], 'y' => $in->accumulative])->prepend(['x' => 0, 'y' => 0])->push(['x' => $report->x->intervals->last()->interval[1] + $report->x->interval_value, 'y' => 1])->toJson() }}"></div>--}}
    ...
    <p class="text-base mb-4">
        Установим уровень значимости $ \alpha $, при котором распределение для выборки согласуется с <strong>показательным
            законом</strong> распределения по критерию Пирсона ($ \chi^2 $).
  ...
  \end{cases}
    ...
    <table class="w-full mb-4">
        ...
        <tr>
            ...
            <td></td>
            <td></td>
            <td class="border px-6 py-4 text-center">$ \chi^2_{набл} = {{ $report->x->intervals->sum('pearson') }}$
            </td>
        </tr>
        </tbody>
    </table>
...
{{--    </p>--}}
    ...
    <p class="text-base mb-4">Вывод.</p>
